...
- [二、数据流（文档处理管线）](#二数据流文档处理管线)
  ...
  - [2.5 审稿增强内容优化的 changes JSON 格式](#25-审稿增强内容优化的-changes-json-格式)
...
**路径 C → B 审稿复核增强**：路径 C 生成的成品文档，可进入路径 B 的内容优化流程做二次审稿复核。路径 B 在生成 `changes` JSON 时，除常规语义语境分析外，还需模拟五角色审稿流程（撰稿人自审→业务审核→文字校对→综合核稿→领导签发），将各角色的优化意见写入 `reason` 和 `reference` 字段。
...
### 2.5 审稿增强内容优化的 changes JSON 格式

路径 B 的内容优化在生成 `changes` JSON 时，`optimized_text` 字段即为**五位审稿角色意见全部合成后的最终可用内容**，`reason` 和 `reference` 字段承担"修改说明"职责，记录各角色的评审依据。

**关键规则**：
- `optimized_text` = 多角色审稿意见融合后的**唯一最终版本**，直接用于替换原文并生成对比文档
- `reason` = 各审稿角色对该段的具体修改意见（仅写实际有建议的角色，不必五个都出现）
- `reference` = 修改所依据的公文写作规范或政策文件
  ...
  "changes": [
    {
      ...
      "reason": "【撰稿人自审】事实准确，但句式可更简洁；【业务审核】业务口径无误，分工边界清晰；【文字校对】'抓紧落实'建议改为'尽快落实'，更符合公文用语规范",
      "style": "庄重严谨",
      "reference": "公文写作规范依据——《党政机关公文处理工作条例》第三章: 公文用语应准确、简洁、庄重、规范"
    }
  ]
}
```
...
| 审稿角色 | 评审维度 | reason 字段中的表述示例 |
|----------|---------|----------------------|
| ①撰稿人自审 | 事实准确、逻辑通顺 | "事实准确，结构合理，建议精简过渡句" |
| ②业务审核 | 业务口径、分工边界 | "业务口径准确，分工责任明确，无需修改" |
| ③文字校对 | 用语规范、序号格式 | "'抓紧'→'尽快'，对标点做全角半角统一" |
| ④综合核稿 | 政策口径、风险排查 | "全局逻辑闭合，政策口径合规，建议补充时限" |
| ⑤领导签发 | 最终审定、是否同意 | "核心观点清晰，同意印发" |

**实践要点**：
- 每段 `optimized_text` 是五位审稿角色意见合成后的**最终输出**，`reason` 仅记录各角色的判断过程
- 若某角色对该段落无意见，则 reason 中不出现该角色的标注
- `optimize-content --apply` 生成的对比文档中，红色标注部分即为 `optimized_text` 相对于 `original_text` 的变更
...
| v1.11.0 | 2026-07-29 | 审稿机制融入路径B/C流程 | optimized_text 审稿合成 | 五角色审稿→reason字段记录 | ARCHITECTURE.md; SKILL.md; review_generator.py |
