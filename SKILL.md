...
| `rule-export` | 🔍 只读 | 导出规则参考文件 |
| `check` | 🔍 只读 | 检查格式是否合规 |
| `parse` | 🔍 只读 | 导出为 JSON 中间表示 |
| `revise` | ✏️ 只改内容 | 内容修订（红色高亮+删除线+修改说明） |
| `md2docx` | 🔧 生成新文件 | Markdown 转公文（含格式排版） |
| `optimize` | 🔧 改格式 | 自动修复字体/字号/行距/边距（支持 --content-only / --format-only 分别处理） |
| `template` | 🔧 生成新文件 | 生成空白公文模板 |
| `generate` | 🔧 生成新文件 | 从 JSON/参数生成公文文档 |
| `header` | 🔧 改格式 | 注入公文版头 |
| `footer` | 🔧 改格式 | 注入公文版记 |
| `pagenum` | 🔧 改格式 | 注入页码 |
| `rule-import` | 🔧 改格式 | 导入自定义规则 |
...
控制台输出汇总：修改建议与说明（共修改 1 处、删除 0 处、新增 0 处）
         1. 【修改】优化措辞 【依据】党组会议审议通过 【行文风格】庄重严谨
```

### 修订说明规范（Agent 填写说明时务必遵守）

每个修改段落后必须附一条独立的修改说明段落，格式要求如下：

**格式规范：**
- 字体：楷体（代码已预设）
- 字号：比正文小1～2级（如正文16pt则说明用12pt）
- 位置：紧跟在修改段落后方
...
| 要素 | 要求 | 示例 |
|------|------|------|
| **操作定性** | 明确说明修改性质：补充/重构/调整/统一/删减/纠正等 | `将"贯彻"调整为"抓好落实"` |
| **具体修改内容** | 用引用或转述方式指出原稿与修改稿的差异 | `在"共同设立专项经费"后补充"并督促承接企业按1:3比例配套研发资金"` |
| **修改依据** | 必须引用具体来源：背景资料、政策原文、行业通用表述、上下文一致性等 | `与背景资料第二部分第三点经费表述保持一致` |
| **效果/意图** | 说明修改后达到什么目的 | `使经费来源表述完整，形成三方合力保障格局` |

> ✅ 完整示例：
> `【修改说明】将"贯彻"调整为"抓好落实"，突出执行力度，与后文会议要求呼应。将"会议"调整为"党组会议"，明确会议层级。修改依据：党组会议审议通过的大型综合性运动会全周期智能管理平台项目汇报。效果：使表述与会议实际层级一致，强化执行要求。`
>
> ✅ 长段示例（重构类）：
> `【修改说明】本段进行了功能性重构：将原罗列问题的写法改为论证课题立项必要性。1.开篇不再定性为"协同机制不健全"，而是从行业共性痛点切入。2.将原三点问题重新整合为"传统模式困境、数据标准缺失、经验基础薄弱"三个层次。3.末尾将"亟需引入"调整为"亟需通过本课题提供坚实保障"。修改依据：背景资料第一部分课题定位与内容中的痛点分析。效果：从"罗列问题"转为"论证必要性"，使段落结构更有说服力。`
>
> ❌ 差的示例：
> `【修改说明】优化措辞 【依据】党组会议`
