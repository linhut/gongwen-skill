...
### 5. 语义级别调用的容错处理（LLM 模糊调用）

AI Agent 经常收到用户含糊的指令（只说"帮我处理这份通知"而不指定类型或参数），此时必须自动容错：

**自动推断文档类型：** 本引擎已内置智能类型解析，当用户未指定 `-t` 时：
- 按文件名关键词自动推断（如文件名含"请示"→ request、"函"→ letter）
- 按文档内容关键词自动推断（如正文含"妥否，请批示"→ request）
- 支持中英文别名和常见拼音缩写（如 "tz"→ notice、"请示"→ request）
- 兜底默认 `notice`，并在终端提示用户

**AI Agent 应主动执行的容错策略：**

| 用户说... | Agent 自动推断 | 无需用户确认的参数 |
|-----------|---------------|-------------------|
| "帮我检查这个文件" | 读取文件 → 自动检测类型 | 输出路径默认 `_优化版` |
| "优化这个通知" | 自动检测类型为 notice | `-t notice` 无需用户确认 |
| "生成一份请示" | 自动检测→ request | 模板路径自动命名 |
| "处理这份请示的格式" | 从文件名/内容自动识别 request | 直接执行 check 后展示结果 |

> ✅ 正确示范："正在分析您的文档...（自动识别为请示类型），发现 2 个格式问题，是否要修复？"
> ❌ 错误示范："请告诉我文档类型"（用户期望的是自动处理，而不是反问）

### 6. 合理默认值原则

所有子命令都已预置合理默认值，AI Agent 在大多数情况下不需要让用户补充参数：

- **输出路径**：自动在原文件名后加 `_优化版` / `_对比` / `_模板`
- **公文类型**：自动检测（文件名 → 内容分析 → notice 兜底）
- **修复范围**：默认全量修复，用户可指定 `--selected-rules` 选择性修复
- **交互确认**：TTY 环境下默认弹出确认，非 TTY/管道 自动跳过
- **非交互环境**：使用 `-y` 标志跳过所有确认提示
