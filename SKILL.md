...
| 路径 | 产物 | diff 标记 | 修改说明 | 颜色标记 | 产出文件名后缀 |
|------|------|-----------|----------|----------|--------------|
| **A**（格式修复） | 干净成品 | 无 | 无 | 无 | `-格式优化` |
| **B**（内容优化） | 差异对比文档 | 有（灰删+红增） | 每段附楷体说明 | 有（#999999 / #E00000） | `-修订对比` |
| **C**（生成模板） | 空白模板 | 无 | 无 | 无 | `-模板` |
...
输出文件名必须遵循以下规则，让用户一眼区分产物类型：

| 路径 | 命名格式 | 示例 |
|------|----------|------|
| A（格式修复） | `{原文档名}-格式优化.docx` | `关于XX的通知-格式优化.docx` |
| B（内容优化） | `{原文档名}-修订对比.docx` | `关于XX的通知-修订对比.docx` |
| C（生成模板） | `{公文类型}-模板.docx` | `通知-模板.docx` |

- 原文档名不含扩展名
- 不允许使用 `{原文档名}_v5.docx`、`{原文档名}-成品.docx` 等无信息量命名
...
生成 `reason` 时，必须按以下三层结构逐段分析，缺一不可：

> **第1层 · 段落类型定位**：先判断这段文字在公文中是什么类型/处于什么位置（如"开头惯用语段"、"问题分析段"、"工作部署段"、"收尾总结段"、"过渡衔接句"等）

> **第2层 · 写作规范判断**：对该类型段落的公文写作规范做判断（如"开头应庄重简洁"、"问题分析应聚焦现状"、"部署应具体可操作"、"过渡应自然流畅"等）

> **第3层 · 具体修改说明**：列出每个修改点——原文是什么、改为什么、理由是什么。用引号标明具体措辞。

**`reference` 字段格式**：引用公文写作规范依据，如"公文汇报材料开头惯用语规范"、"公文'问题分析'部分写作规范——问题与建议应分列"。
...
```json
{
  ...
  "reason": "\"紧扣\"改为\"深入贯彻\"更符合公文庄重语体；\"省委省政府\"中间加顿号保持规范性；\"落实\"前加\"全面\"增强力度；\"研究有关情况\"改为\"研究进展情况\"更精准",
  "reference": "公文汇报材料开头惯用语规范——开头应庄重简洁、用语规范、目的明确"
}
```
